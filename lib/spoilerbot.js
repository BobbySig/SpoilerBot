...
class SpoilerBot {
  ...
  processSpoiler(msg) {
    ...
    let spoilerMsg = msg.content.substring(this.strings.CMD_SPOILER.length + 1);
    let spoilerBreakdown = spoilerMsg.split(this.strings.DELINIATOR);
    ...
    let spTitle = spoilerBreakdown[0].trim();
    ...
    let spSpoiler = spoilerBreakdown[1].trim();
    let spEncoded = ROT13.Cipher().crypt(spSpoiler);
    let spURL = this.strings.DECODE_URL_BASE + encodeURIComponent(spEncoded);
  ...
  sendTitleTooLongMsg(msg) {
    let bot = this;
    let errMsg = this.errorMsg(this.strings.OVERLENGTH_TITLE_HEAD,
  ...
  sendSpoilerTooLongMsg(msg) {
    let bot = this;
    let errMsg = this.errorMsg(this.strings.OVERLENGTH_SPOILER_HEAD,
